...
<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <h2 class="text-3xl font-bold mb-6 text-gray-800">Papelera de Productos</h2>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider max-w-xs">Descripción</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Precio</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider max-w-xs">Motivo de eliminación</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-700">{{ $producto->nombre }}</td>
                    <td class="px-6 py-4 max-w-xs whitespace-nowrap text-sm text-gray-600 truncate" title="{{ $producto->descripcion }}">
                        {{ $producto->descripcion }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">${{ number_format($producto->precio, 2) }}</td>
                    <td class="px-6 py-4 max-w-xs whitespace-nowrap text-sm text-gray-600 truncate" title="{{ $producto->motivo_eliminacion }}">
                        {{ $producto->motivo_eliminacion }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                        {{ $producto->deleted_at->setTimezone(config('app.timezone'))->format('d/m/Y H:i') }}
                    </td>
                ...
                <div id="restaurarModal-{{ $producto->id }}"
                    ...
                    <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
                        <h3 class="text-xl font-bold mb-4 text-gray-800">Restaurar Producto</h3>
                    ...
                    <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
                        ...
                        <form method="POST" action="{{ route('productos.eliminarDefinitivo', $producto->id) }}">
                            ...
                            <label class="flex items-center mb-4 text-sm text-gray-700">
                                <input type="checkbox" name="confirmacion" class="mr-2" onchange="document.getElementById('btnDef-{{ $producto->id }}').disabled = !this.checked">
                            ...
                            <div class="flex justify-end space-x-3">
                                <button type="button" onclick="closeModal('eliminarDefinitivo-{{ $producto->id }}')"
                                    class="px-4 py-2 bg-gray-200 rounded-md hover:bg-gray-300 focus:outline-none">Cancelar</button>
                                <button type="submit" id="btnDef-{{ $producto->id }}" disabled
                ...
                @empty
                <tr>
                    <td colspan="6" class="px-6 py-8 text-center text-gray-500 italic">No hay productos en papelera.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
