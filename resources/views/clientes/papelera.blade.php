...
<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <h2 class="text-3xl font-bold mb-6 text-gray-800">Papelera de Clientes</h2>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-700">{{ $cliente->nombre }}</td>
                    ...
                    <td class="px-6 py-4 max-w-xs whitespace-nowrap text-sm text-gray-600 truncate" title="{{ $cliente->motivo_eliminacion }}">
                        {{ $cliente->motivo_eliminacion }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                        {{ $cliente->deleted_at->setTimezone(config('app.timezone'))->format('d/m/Y H:i') }}
                    </td>
                ...
                <div id="restaurarModal-{{ $cliente->id }}" 
                    ...
                    <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
                        <h3 class="text-xl font-bold mb-4 text-gray-800">Restaurar Cliente</h3>
                    ...
                    <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
                        ...
                        <form method="POST" action="{{ route('clientes.eliminarDefinitivo', $cliente->id) }}">
                            ...
                            <label class="flex items-center mb-4 text-sm text-gray-700">
                                <input type="checkbox" name="confirmacion" class="mr-2" onchange="document.getElementById('btnDef-{{ $cliente->id }}').disabled = !this.checked">
                            ...
                            <div class="flex justify-end space-x-3">
                                <button type="button" onclick="closeModal('eliminarDefinitivo-{{ $cliente->id }}')"
                                    class="px-4 py-2 bg-gray-200 rounded-md hover:bg-gray-300 focus:outline-none">Cancelar</button>
                                <button type="submit" id="btnDef-{{ $cliente->id }}" disabled
                ...
                @empty
                <tr>
                    <td colspan="6" class="px-6 py-8 text-center text-gray-500 italic">No hay clientes en papelera.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
